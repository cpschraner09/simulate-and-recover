...
def compute_forward_stats(a, v, t):
    ...
    # (this is a simplified version; adjust according to your lecture slides)
    M_pred = t + (a / (2*v)) * ((1 - y) / (1 + y))
    
    # Equation (3): Predicted variance RT
    # (again, simplified; refer to the slides for the exact form)
...
def simulate_summary_stats(a, v, t, N):
    ...
    # Simulate observed number of correct responses T_obs
    correct_trials = np.random.binomial(N, R_pred)
    R_obs = correct_trials / N
    
    # Simulate observed mean RT M_obs
    # Standard deviation is sqrt(V_pred / N), numpy requires SD not variance which is V_pred / N
    M_obs = np.random.normal(M_pred, np.sqrt(V_pred / N))
    
    # Simulate observed variance RT V_obs
...
if __name__ == "__main__":
    # Define some example 'true' parameters:
